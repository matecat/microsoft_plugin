;(function (SF, QA_GLOSSARY) {
  ...
  $.extend(UI, {
    showFixWarningsModal: function () {
      ModalsActions.showModalComponent(ConfirmMessageModal, {
        ...
        cancelCallback: () => UI.markJobAsComplete(),
        text: 'Unresolved glossary and/or tag issues are preventing you from completing your translation. <br>Please fix the issues.',
      })
    },
    showFixWarningsOnDownload(continueDownloadFunction) {
      ModalsActions.showModalComponent(ConfirmMessageModal, {
        ...
        text: 'Unresolved glossary and/or tag issues may prevent downloading your translation. Please fix the issues.',
      })
    },
  })
  ...
  $('body').on(
    'click',
    '.modal[data-name=markJobAsComplete] .x-popup',
    function (e) {
      e.preventDefault()
      e.stopPropagation()
  ...
  function overrideSegmentsMatches(SegmentTabMatches) {
    SegmentTabMatches.prototype.processMatchCallback = function (item) {
      if (
        item.percentText === '100%' &&
        item.tm_properties &&
        item.tm_properties.length > 0
      ) {
  ...
  SegmentTabMessages.prototype.excludeMatchingNotesRegExp = new RegExp(
    /(adjWordcount|curWordcount)/m,
  )
